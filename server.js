...
const EMAIL_MODE = process.env.EMAIL_MODE || "demo"; // demo | resend
...
function makePersonId() {
  ...
  return `PID-${100245 + Number(row.n)}`;
}
function makeOtp() {
...
function orgAuth(req, res, next) {
  auth(req, res, () => {
    if (req.auth.role !== "organization") return res.status(403).json({ error: "Organization access required" });
    next();
  });
}
function personAuth(req, res, next) {
  auth(req, res, () => {
    if (req.auth.role !== "person") return res.status(403).json({ error: "Person access required" });
    next();
  });
}
...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "ConnectID API", time: now() });
});
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (!name || !cleanEmail.endsWith("@gmail.com") || !password || password.length < 6) {
...
app.get("/api/me", personAuth, (req, res) => {
  const user = db.prepare("SELECT id,person_id,name,email,mobile,email_verified,created_at FROM users WHERE id=?").get(req.auth.sub);
  res.json(user);
});

app.get("/api/documents", personAuth, (req, res) => {
  const rows = db.prepare("SELECT id,original_name,document_type,extracted_json,created_at FROM documents WHERE user_id=? ORDER BY id DESC").all(req.auth.sub);
...
app.post("/api/documents", personAuth, upload.single("document"), (req, res) => {
  ...
  // Demo OCR adapter: keeps the API ready for a real OCR provider/library.
  // The extracted values can be replaced by Tesseract.js or another OCR service.
  const extracted = {
  ...
  const result = db.prepare(`
    INSERT INTO documents(user_id,original_name,document_type,extracted_json,created_at)
    VALUES(?,?,?,?,?)
  `).run(req.auth.sub, req.file.originalname, extracted.DocumentType, JSON.stringify(extracted), now());
...
app.post("/api/access-requests", orgAuth, (req, res) => {
  ...
  db.prepare(`
    INSERT INTO notifications(person_id,title,message,created_at)
    VALUES(?,?,?,?)
...
// Seed a demo organization for the prototype.
