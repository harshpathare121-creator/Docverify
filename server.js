...
const emptyDb = () => ({
  meta: { nextUserId: 1, nextOrgId: 1, nextAdminId: 1, nextDocumentId: 1, nextRequestId: 1, nextNotificationId: 1, nextAuditId: 1, nextOtpId: 1 },
...
function personId(){ let max=100244; for(const u of db.users){ const n=Number(String(u.person_id||'').replace('PID-','')); if(Number.isFinite(n)) max=Math.max(max,n); } return `PID-${max+1}`; }
...
app.post('/api/access-requests',orgAuth,(req,res)=>{
  ...
  const person=findUserByPerson(personId); if(!person||!person.email_verified)return sendError(res,404,'Verified Person ID not found.'); if(!requestedData.length||!purpose)return sendError(res,400,'Select data and enter a purpose.');
  const r={id:next('nextRequestId'),person_id:personId,organization_id:Number(req.auth.sub),requested_data:JSON.stringify(requestedData),purpose,duration,status:'Pending',otp_hash:null,otp_expires_at:null,created_at:now(),approved_at:null,granted_at:null,revoked_at:null}; db.access_requests.push(r); const o=orgById(req.auth.sub); notify(personId,'New access request',`${o.name} requested: ${requestedData.join(', ')}. Purpose: ${purpose}.`); audit(personId,'ACCESS_REQUESTED',`Requested ${requestedData.join(', ')}`,o.id); res.json({ok:true,requestId:r.id});
});
